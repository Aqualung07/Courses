...
/*   Write a function after that takes the number of times the callback needs to be called before 
  ...
  */
  ...
  // const delayedCall = delay(() => {console.log("miau"), 5000});
  // delayedCall();
  ...
  */
  function rollCall(names) {
    ...
    const threshold = names.length;
    let counter = 0;
    
    return () => {
      if (counter >= threshold) return console.log('Everyone accounted for');
  ...
  */
  function saveOutput(func, magicWord) {
    ...
    return (input) => {
      if (input == password) return storedOutputs;
      const storedResult = func(input);
      storedOutputs[input] = storedResult;
      return storedResult;
    }
  }
  ...
  */
  function cycleIterator(array) {
    const reset = array.length;
    let counter = 0;
    
    return () => {
      if (counter >= reset) counter = 0;
      const output = array[counter];
      counter++;
      return output;
    }
  ...
  */
  function censor() {
    ...
    return (str1, str2 = undefined) => {
      if (!str2) {
        for (let prop in pairedStr) {
          str1 = str1.replace(prop, pairedStr[prop]);
        }
        return str1;
      }
      return pairedStr[str1] = str2;
    }
  ...
  */
  function createSecretHolder(secret) {
    ...
    return ({
        getSecret: () => privateValue,
        setSecret: input => privateValue = input
    })
  }
  ...
  */
  function roulette(num) {
      const threshold = num-1;
    let counter = 0;
    
    return () => {
      let returnStr = "pick a number to play again";
      if (counter < threshold) returnStr = "spin";
      if (counter == threshold) returnStr = "win";
      ++counter;
      return returnStr;
    }
  }
  ...
  */
  function average() {
    ...
    return (num = undefined) => {
      if (!avg && !num) return 0;
      if (num && numberList.indexOf(num) == -1) {
        numberList.push(num);  
      }
      const total = numberList.reduce((sum, item) => { return sum += item }, 0);
  ...
  function makeFuncTester(arrOfTests) {
    ...
    return (callback) => {
      let returnValue = true;
        ...
        if (callback(tuple[0]) != tuple[1]) returnValue = false;
      });
  ...
  */
  function makeHistory(limit) {
    ...
    return (string) => {
      const success = `${string} done`;
      
      if (string == 'undo') {
        if (lastItem.length == 0) return "nothing to undo";
        return `${lastItem.pop()} undone`;
      }
      
      lastItem.push(string);
      if (lastItem.length <= limit) return success;
      lastItem.shift();
      return success;
    }
  }
  ...
  /*
    ...
    the previous PLAYER function left off. 
...
function blackjack(array) {
    ...
    function dealer(num1, num2) {
      ...
      if (cardIndex >= array.length) return () => 'No more room for players';
      
        function player() {
            // BEGIN PLAYER FUNCTION
            const busted = 'you are done';
            if (!bust) {
                if (round == 0) {
                    sum += num1+num2;
                } else {
  ...
  // /*** PLAYER 4 ***/
  /*const i_ALSO_like_to_live_dangerously2 = deal(1, 1);
   console.log(i_ALSO_like_to_live_dangerously2()); // => should log 2
   console.log(i_ALSO_like_to_live_dangerously2()); // => should log 10
   console.log(i_ALSO_like_to_live_dangerously2()); // => should log 19
   console.log(i_ALSO_like_to_live_dangerously2()); // => should log 'bust'
   console.log(i_ALSO_like_to_live_dangerously2()); // => should log 'you are done!
  
  // /*** PLAYER 5 ***/
  /*const i_ALSO_like_to_live_dangerously3 = deal(1, 1);
   console.log(i_ALSO_like_to_live_dangerously3()); // => should log 2
   console.log(i_ALSO_like_to_live_dangerously3()); // => should log 12
   console.log(i_ALSO_like_to_live_dangerously3()); // => should log 16
   console.log(i_ALSO_like_to_live_dangerously3()); // => should log 21
   console.log(i_ALSO_like_to_live_dangerously3()); // => should log 'bust'*/
  
  // /*** PLAYER 6 ***/
  /*const i_ALSO_like_to_live_dangerously4 = deal(1, 1);
   console.log(i_ALSO_like_to_live_dangerously4()); // => should log 2
   console.log(i_ALSO_like_to_live_dangerously4()); // => should log 9
   console.log(i_ALSO_like_to_live_dangerously4()); // => should log 13
   console.log(i_ALSO_like_to_live_dangerously4()); // => should log 'bust'
   console.log(i_ALSO_like_to_live_dangerously4()); // => should log 'you are done!*/
  
  // /*** PLAYER 7 ***/
  /*const i_ALSO_like_to_live_dangerously5 = deal(1, 1);
   console.log(i_ALSO_like_to_live_dangerously5()); // => should log 2
   console.log(i_ALSO_like_to_live_dangerously5()); // => should log 8
   console.log(i_ALSO_like_to_live_dangerously5()); // => should log 18
   console.log(i_ALSO_like_to_live_dangerously5()); // => should log 'bust'
   console.log(i_ALSO_like_to_live_dangerously5()); // => should log 'you are done!*/
  
  // /*** PLAYER 8 ***/
  /*const i_ALSO_like_to_live_dangerously6 = deal(1, 1);
   console.log(i_ALSO_like_to_live_dangerously6()); // => should log 'No more room for players'
   console.log(i_ALSO_like_to_live_dangerously6()); // => should log 'No more room for players'
   console.log(i_ALSO_like_to_live_dangerously6()); // => should log 'No more room for players'
   console.log(i_ALSO_like_to_live_dangerously6()); // => should log 'No more room for players'
   console.log(i_ALSO_like_to_live_dangerously6()); // => should log 'No more room for players'*/
